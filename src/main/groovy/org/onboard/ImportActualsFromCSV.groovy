...
class ImportActualsFromCSV {
        ...
        RowValidation rowValidation = new RowValidation(
                ...
                [
                        ...
                        new FieldValidation(
                                ["z_cost_raw"]
                                , "z_cost_cl"
                                , [ImportActualsFromCSV.&isMoney]
                                , ImportActualsFromCSV.&moneyToNumber
                        ),
                        new FieldValidation(
                                ["z_units_raw"]
                                , "z_units_cl"
                        ...
                        new FieldValidation(
                                ["z_res_code_raw"]
                                , "z_res_code_cl"
                                , [ValidationRule.&isNotEmpty, this.&isValidResource]
                                , OutputFormatter.&asDynamicLookupValue.curry(securityIdentifier, lookupService, "LOOKUP_RESOURCES", "RESOURCE ID", [:], ["RESOURCE ID"])
                        ),
                        new FieldValidation(
                                ["z_res_code_cl"]
                                , "z_res_id_cl"
    ...
    String getResType(Object... inputs) {
        if (inputs) {
    ...
    Validation isValidResource(Object... inputs) {
        def res = sql.firstRow("select id from srm_resources where unique_name = ${inputs[0]}")?.id
        if (res != null) {
            return ValidationRule.isNotEmpty(inputs[0])
        }
